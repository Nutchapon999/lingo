...
export const getLessonPercentage = cache(async () => {
  ...
  const completedChallenge = lesson.challenges.filter((challenge) => challenge.completed);
  const percentage = Math.round((completedChallenge.length / lesson.challenges.length) * 100);
